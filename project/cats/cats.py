...
def accuracy(typed, source):
    ...
    if typed_words == [] and source_words == []:
        return 100.0
    l = min(len(typed_words), len(source_words))
    right = 0
    for i in range(l):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    while i >= 0:
        ...
        if diff <= limit and diff <= min_len:
            ans = word_list[i]
            min_len = diff
        i -= 1
    return ans
...
def feline_fixes(typed, source, limit):
    ...
    if limit < 0:
        return 0
    if typed[i - 1:i] != source[i - 1:i]:
        return 1 + feline_fixes(typed[:i - 1], source[:i - 1], limit - 1) + ans
    return feline_fixes(typed[:i - 1], source[:i - 1], limit) + ans
    # END PROBLEM 6
...
#    assert False, 'Remove this line'
    ...
    if limit < 0:  # Fill in the condition
        ...
        return 0
        # END
    if start in goal or goal in start:  # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        return diff
        # END
...
def report_progress(typed, prompt, user_id, upload):
    ...
    a = len(typed)
    b = len(prompt)
    while num < a and typed[num] == prompt[num]:
        num += 1
        
    rate = num / b
    w = {'id':user_id, 'progress':rate}
    upload(w)
    return rate
    # END PROBLEM 8
