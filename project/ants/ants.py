...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        "*** YOUR CODE HERE ***"
        if exit:
            exit.entrance = self                    #exit指的是从self出去可以到exit,所以self是exit的entrance
        # END Problem 2
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            ant = place.ant
            if ant.is_container and ant.can_contain(self):
                ant.store_ant(self)
            elif not ant.is_container and self.is_container: 
                self.store_ant(ant)
                place.ant = self
            else:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        damage_to_bee = amount if self.health - amount > 0 else amount + self.damage
...
# The HungryAnt Class
class HungryAnt(Ant):
    """which will select a random Bee from its place and deal damage to the Bee by eating it whole. After eating a Bee 
      , a HungryAnt must spend 3 turns chewing before being able to eat again.
    """
    name = 'Hungry'
    food_cost = 4
    chewing_turns = 3
    ...
    def action(self, gamestate):
        ...
        while double_ants_place:   # When place is not none, ants in place double damages      
            if double_ants_place.ant:
                if not double_ants_place.ant.is_double:
                ...
                if double_ants_place.ant.is_container:
                    if double_ants_place.ant.ant_contained:
                        if not double_ants_place.ant.ant_contained.is_double:  
